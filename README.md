...
`bsrvcore` client APIs, using its three-stage Assembler/Builder/Task
architecture with optional proxy support.

`boai` requires `bsrvcore` `v0.18.2`. When `boai` falls back to automatic
dependency fetching, it defaults to `bsrvcore v0.18.2`.
...
cmake -S . -B build \
  ...
  -DBOAI_BSRVCORE_GIT_TAG=v0.18.2
```
