...
using boai::completion::OaiCompletionFactory;
using boai::completion::OaiCompletionInfo;
using boai::completion::OaiCompletionState;
...
std::optional<std::filesystem::path> FindConfigPath() {
  ...
  for (int i = 0; i < 8; ++i) {
    fs::path candidate =
        current / ".artifacts" / "secrets" / "boai_completion_test_config.json";
...
TEST(OaiCompletionIntegrationTest, DeepSeekCompletionSuccessWhenConfigPresent) {
  ...
  if (!config.has_value()) {
    GTEST_SKIP()
        << "Missing valid .artifacts/secrets/boai_completion_test_config.json";
  }
